...
async function isOwnerOrSudo(senderId, sock = null, chatId = null) {
    const ownerSetting = settings.ownerNumber;
    const senderIdClean = cleanJid(senderId);

    console.log(`[AUTH DEBUG] Checking Owner: Sender=${senderId} (${senderIdClean}) vs OwnerSetting=${ownerSetting}`);

    let isOwner = false;
    if (Array.isArray(ownerSetting)) {
        isOwner = ownerSetting.some(num => cleanJid(num) === senderIdClean);
    } else {
        isOwner = cleanJid(ownerSetting) === senderIdClean;
    }
...
function isOwnerOnly(senderId) {
    ...
    if (Array.isArray(ownerSetting)) {
        return ownerSetting.some(num => cleanJid(num) === senderIdClean);
    }
    return cleanJid(ownerSetting) === senderIdClean;
}
